...
namespace
{
	...
	public:
		ScopedInstrumentPromptDisable()
			: _config(ARDOUR::Config), _ask_replace(false), _ask_setup(false), _active(false)
		{
			if (!_config)
			{
				return;
...
MCPHttpServer::MCPHttpServer(ARDOUR::Session &session, uint16_t port, int debug_level, PBD::EventLoop *event_loop)
	: _session(session), _port(port), _debug_level(clamp_debug_level(debug_level)), _event_loop(event_loop), _context(0), _running(false)
{
...
void MCPHttpServer::set_debug_level(int level)
{
	_debug_level.store(clamp_debug_level(level));
}

int MCPHttpServer::debug_level() const
{
...
void MCPHttpServer::erase_client(struct lws *wsi)
{
	ClientMap::iterator it = _clients.find(wsi);
	if (it != _clients.end())
	{
...
int MCPHttpServer::handle_http(struct lws *wsi, ClientContext &ctx)
{
	ctx.mcp_post = false;
	ctx.have_response = false;
...
int MCPHttpServer::handle_http_body(struct lws * /*wsi*/, ClientContext &ctx, void *in, size_t len)
{
	if (!ctx.mcp_post)
	{
		return 0;
...
int MCPHttpServer::handle_http_body_completion(struct lws *wsi, ClientContext &ctx)
{
	if (!ctx.mcp_post)
	{
		return send_http_status(wsi, 400);
...
int MCPHttpServer::handle_http_writeable(struct lws *wsi, ClientContext &ctx)
{
...
int MCPHttpServer::send_http_status(struct lws *wsi, unsigned int status)
{
	lws_return_http_status(wsi, status, 0);
	return -1;
}

int MCPHttpServer::send_json_headers(struct lws *wsi)
{
...
int MCPHttpServer::write_json_response(struct lws *wsi, ClientContext &ctx)
{
...
int MCPHttpServer::callback(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len)
{
...
int MCPHttpServer::lws_callback(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len)
{
